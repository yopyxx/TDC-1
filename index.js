...
// ================== 데이터 구조 ==================
// ✅ 변경 포인트:
// - u.daily[date] 에 admin/extra 뿐 아니라 "입력 원본 필드" + 사진URL까지 저장
let data = {
...
// ================== ✅ (신규) “하루치 보고자 전원”을 한 장의 표로 구글시트에 작성 ==================
...
function buildDailyTableValues(dateStr) {
  // ✅ “그 날 명령어 사용한 사람만” 모으기
  // 소령 테이블 헤더(스크린샷 느낌의 열 구성)
  const majorHeader = [
    '단위/명', '닉네임', '권한 지급', '랭크 변경', '팀변경', '총 행정 건수',
    '스카웃/모집시험', '인게임 시험', '총점수',
    '유저ID', '증거사진(최대10)'
  ];
  ...
  // 중령 테이블 헤더
  const ltcolHeader = [
    '단위/명', '닉네임', '역할 지급', '인증 처리', '감찰', '서버 역할', '총 행정 건수',
    '피드백', '인게임 시험', '코호스트', '총점수',
    '유저ID', '증거사진(최대10)'
  ];

  const majorRows = [];
  for (const [uid, u] of Object.entries(data.소령.users || {})) {
    const d = u?.daily?.[dateStr];
    if (!d || !d.__reported) continue; // ✅ 그 날 보고한 사람만

    const input = d.__input || {};
    const photoUrls = Array.isArray(d.__photos) ? d.__photos : [];
    const photoCells = photoUrls.slice(0, 10).map(x => toImageFormula(x)).join(' , ');

    // ✅ 원자료 총점 = admin(총행정건수) + extra(추가점수) (현재 코드 기준)
    const rawTotal = (d.admin || 0) + (d.extra || 0);

    majorRows.push([
      '1명', // 스크린샷처럼 유지(원하면 여기 단위명으로 바꿔도 됨)
      u.nick || uid,
      input.권한지급 ?? 0,
      input.랭크변경 ?? 0,
      input.팀변경 ?? 0,
  ...
  const ltcolRows = [];
  for (const [uid, u] of Object.entries(data.중령.users || {})) {
    const d = u?.daily?.[dateStr];
    if (!d || !d.__reported) continue;
  ...
  values.push([`소령 (${dateStr})`]);
  values.push(majorHeader);
  if (majorRows.length) values.push(...majorRows);
  else values.push(['(해당 일자 보고자 없음)']);

  values.push([]);
  values.push([]);
  values.push([`중령 (${dateStr})`]);
  values.push(ltcolHeader);
  if (ltcolRows.length) values.push(...ltcolRows);
  else values.push(['(해당 일자 보고자 없음)']);

  return values;
}

async function writeDailyReportTab(dateStr) {
  // 탭 이름 = 날짜 (한 장)
  const tabTitle = dateStr;
  await ensureSheetTabExists(tabTitle);

  const sheets = getSheetsClient();
  const values = buildDailyTableValues(dateStr);

  // ✅ 기존 내용 지우고 A1부터 전체 다시 작성
  ...
  console.log(`📄 일일 표 작성 완료: 탭 "${tabTitle}" (${dateStr})`);
}

// ================== 자동 스냅샷/표 작성 ==================
async function runDailyAutoReset() {
  ...
  // ✅ (신규) 어제 보고자 전원 “한 장의 표”로 구글시트에 정리
  try {
    await writeDailyReportTab(y);
  } catch (e) {
    console.error('❌ 일일 표(구글시트) 작성 실패:', e);
  }
...
client.on('interactionCreate', async interaction => {
  ...
  if (cmd === '소령행정보고' || cmd === '중령행정보고') {
    ...
    // ✅ (중요) “그 날 보고했다” 표시 + 입력 원본 + 사진 URL 저장
    u.daily[date].__reported = true;
    u.daily[date].__input = input;
    u.daily[date].__photos = photoAttachments.slice(0, 10).map(a => a.url);

    // 기존 누적 로직 유지 (같은 날 여러번 보고하면 합산)
...
3) 스프레드시트 ID: 1EhC_xDBXR7mm7_KdVPp2dKPjNSKClcEEHIZT6ZPvfaQ
